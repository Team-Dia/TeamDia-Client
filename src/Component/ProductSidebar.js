...
const ProductSidebar = ({ setPrevCategory }) => {
    ...
    const handleApplyFilter = () => {
        const filterParams = { category: selectedCategory, subCategory: selectedSubCategory };
    ...
    return (
        <aside className="product-sidebar">
                ...
                {selectedPrice === "custom" && (
                    <div className="price-inputs">
                        <input 
                            type="number" 
                            value={customPrice[0]} 
                            onChange={e => setCustomPrice([+e.target.value || 0, customPrice[1]])} 
                        />
                        <span>~</span>
                        <input 
                            type="number" 
                            value={customPrice[1]} 
                            onChange={e => setCustomPrice([customPrice[0], +e.target.value || 0])} 
                        />
                    </div>
                )}
            </div>
            ...
            <div className="filter-buttons">
                <button className="reset-button" onClick={handleResetFilter}>초기화</button>
                <button className="apply-button" onClick={handleApplyFilter}>적용</button>
            </div>
        </aside>
    );
};
